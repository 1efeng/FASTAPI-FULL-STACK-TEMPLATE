...
PostgreSQL 默认映射到宿主机 5432。如果端口被占用,可以在 .env 中设置 POSTGRES_HOST_PORT=5433;容器内部仍使用 5432。

FastAPI 把构建好的前端和 API 作为一个应用在 `http://localhost:8000` 提供服务。API 路由位于 `/api/v1` 下。
...
当你部署到生产环境(或预发环境)时,应用使用一个域名。前端在 `/` 提供,API 在 `/api/v1` 下。
