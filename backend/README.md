...
确保你的编辑器使用正确的 Python 虚拟环境,解释器位于 backend/.venv/bin/python; 项目要求 Python 3.14.6。
...
* Alembic loads all ORM models through app/db/models.py, which is imported by app/alembic/env.py and supplies Base.metadata for autogenerate. Add every new model to app/db/models.py.
