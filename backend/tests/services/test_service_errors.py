...
async def _create_user(db: AsyncSession, *, is_superuser: bool = False, is_active: bool = True) -> tuple[User, str]:
    password = random_lower_string()
    user = await UserService(db).create_user(UserCreate(email=random_email(), password=password, is_superuser=is_superuser, is_active=is_active))
